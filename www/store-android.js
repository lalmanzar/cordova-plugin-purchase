...
(function() {
    ...
    store.Product = function(options) {
        ...
        var defaultGroup =
            this.type === store.PAID_SUBSCRIPTION ? "default" : "";
        ...
        this.description =
            options.description || options.localizedDescription || null;
        ...
        this.introPriceNumberOfPeriods =
            options.introPriceNumberOfPeriods || null;

        ///  - `product.introPriceSubscriptionPeriod` - Period for the introductory price ("Day", "Week", "Month" or "Year")
        this.introPriceSubscriptionPeriod =
            options.introPriceSubscriptionPeriod || null;
    ...
    store.Product.prototype.verify = function() {
        ...
        var tryValidation = function() {
            ...
            store._validator(that, function(success, data) {
                ...
                if (dataTransaction) {
                    that.transaction = Object.assign(
                        that.transaction || {},
                        dataTransaction
                    );
                ...
                if (success) {
                    if (that.expired) that.set("expired", false);
                    store.log.debug(
                        "verify -> success: " + JSON.stringify(data)
                    );
                    store.utils.callExternal(
                        "verify.success",
                        successCb,
                        that,
                        data
                    ...
                    if (getData(data, "latest_receipt")) {
                        ...
                        store.log.debug(
                            "verify -> server did use the latest_receipt, no retries"
                        );
                        nRetry = 999999;
                    }
                    ...
                    if (data.code === store.PURCHASE_EXPIRED) {
                        ...
                        store.log.debug(
                            "validation failed, no retrying, trigger an error"
                        );
                        store.error(err);
        ...
        function extractTransactionFields(that) {
            ...
            if (t.type === "ios-appstore" && t.expires_date_ms) {
                ...
                t.type === "android-playstore" &&
                t.expiryTimeMillis > 0
            ) {
            ...
            if (t.expiryDate) that.expiryDate = new Date(t.expiryDate);
            if (t.lastRenewalDate)
                that.lastRenewalDate = new Date(t.lastRenewalDate);
            if (t.renewalIntent) that.renewalIntent = t.renewalIntent;
            return t;
        }
    };
...
(function() {
    ...
    store.register = function(product) {
        if (!product) return;
        if (!product.length) store.register([product]);
        else registerProducts(product);
    };
    ...
    function runValidation() {
        ...
        Object.keys(byProduct).forEach(function(productId) {
            ...
            if (!product.additionalData.applicationUsername) {
                product.additionalData.applicationUsername = store._evaluateApplicationUsername(
                    product
                );
            }
            if (!product.additionalData.applicationUsername) {
            ...
            store.utils.ajax({
                ...
                success: function(data) {
                    store.log.debug(
                        "validator success, response: " + JSON.stringify(data)
                    );
...
(function() {
    ...
    store.refresh = function() {
        ...
        store.log.debug(
            "refresh -> checking products state (" +
        ...
        for (var i = 0; i < store.products.length; ++i) {
            ...
            store.log.debug(
                "refresh -> product id " + p.id + " (" + p.alias + ")"
            );
            ...
            // because validation will fail with a connection timeout.
                ...
                p.state === store.OWNED &&
                (p.type === store.FREE_SUBSCRIPTION ||
                    p.type === store.PAID_SUBSCRIPTION)
            )
...
(function() {
    ...
    store._evaluateApplicationUsername = stringOrFunction(
        "applicationUsername"
    );

    function stringOrFunction(key) {
        return function(product) {
            if (typeof store[key] === "function") return store[key](product);
            return store[key] || "";
        };
    }
})();
...
(function() {
    ...
    store.Product.prototype.stateChanged = function() {
        ...
        store.getGroup(this.group).forEach(
    ...
    store._queries = {
        ...
        triggerAction: function(action, args) {
            ...
            if (cbs) {
                ...
                store._queries.callbacks.byQuery[action] = cbs.filter(
                    isNotOnce
                );
            }
            ///
        ...
        triggerWhenProduct: function(product, action, args) {
            ...
            if (
                product &&
                product.type &&
                (product.type === store.FREE_SUBSCRIPTION ||
                    product.type === store.PAID_SUBSCRIPTION)
            )
...
(function() {
    ...
    store.utils = {
        ...
        logError: function(context, err) {
            store.log.warn(
                "A callback in '" + context + "' failed with an exception."
            );
            if (typeof err === "string") store.log.warn("           " + err);
            else if (err) {
                if (err.fileName)
                    store.log.warn(
        ...
        ///
        /* eslint-disable */ // Based on the work of Jeff Mott, who did a pure JS implementation of the MD5 algorithm that was published by Ronald L. Rivest in 1991.
        ...
        md5: (function md5(r) {
            function n(o) {
                ...
                return (
                    r[o].call(e.exports, e, e.exports, n), (e.l = !0), e.exports
                );
            }
            var t = {};
            return (
...
(function() {
    ...
    InAppBilling.prototype.init = function(
        success,
        fail,
        options,
        skus,
    ...
    InAppBilling.prototype.listener = function(msg) {
        ...
        if (
            msg.type === "purchasesUpdated" &&
            this.options.onPurchasesUpdated
        ) {
            this.options.onPurchasesUpdated(msg.data.purchases);
        }
        if (
            msg.type === "purchaseConsumed" &&
            this.options.onPurchaseConsumed
        ) {
    ...
    function extendAdditionalData(ad) {
        ...
        if (!additionalData.accountId && additionalData.applicationUsername) {
            additionalData.accountId = store.utils.md5(
                additionalData.applicationUsername
            );
        }
        return additionalData;
    }
    InAppBilling.prototype.buy = function(
        success,
        fail,
        productId,
        additionalData
    ...
    InAppBilling.prototype.subscribe = function(
        success,
        fail,
        productId,
        additionalData
    ...
    InAppBilling.prototype.consumePurchase = function(
        success,
        fail,
        productId,
        transactionId,
        ...
        return cordova.exec(
            ...
            "InAppBillingPlugin",
            "consumePurchase",
            [productId, transactionId, developerPayload]
        );
    };
    InAppBilling.prototype.acknowledgePurchase = function(
        success,
        fail,
        productId,
        transactionId,
...
(function() {
    ...
    function init() {
        ...
        for (var i = 0; i < store.products.length; ++i) {
            ...
            else inAppSkus.push(store.products[i].id);
        }

        store.inappbilling.init(
            iabReady,
            function(err) {
                initialized = false;
                store.error({
    ...
    function iabLoaded(validProducts) {
        ...
        for (i = 0; i < validProducts.length; ++i) {
            ...
            if (vp.productId) p = store.products.byId[vp.productId];
            else p = null;

            if (p) {
                var subscriptionPeriod = vp.subscriptionPeriod
                    ? vp.subscriptionPeriod
                    : "";
                var introPriceSubscriptionPeriod = vp.introductoryPricePeriod
                    ? vp.introductoryPricePeriod
                    : "";
                var introPriceNumberOfPeriods = vp.introductoryPriceCycles
                    ? vp.introductoryPriceCycles
                    : 0;
                ...
                introPriceSubscriptionPeriod = normalizeISOPeriodUnit(
                    introPriceSubscriptionPeriod
                );

                var parsedSubscriptionPeriod = {};
                if (subscriptionPeriod) {
                    parsedSubscriptionPeriod.unit = normalizeISOPeriodUnit(
                        subscriptionPeriod
                    );
                    parsedSubscriptionPeriod.count = normalizeISOPeriodCount(
                        subscriptionPeriod
                    );
                }
                ...
                p.set({
                    ...
                    billingPeriod:
                        parsedSubscriptionPeriod.count ||
                        vp.billing_period ||
                        null,
                    billingPeriodUnit:
                        parsedSubscriptionPeriod.unit ||
                        vp.billing_period_unit ||
                        null,
                    description: vp.description,
                    currency: vp.price_currency_code || "",
                    introPrice: vp.introductoryPrice
                        ? vp.introductoryPrice
                        : "",
                    introPriceMicros: vp.introductoryPriceAmountMicros
                        ? vp.introductoryPriceAmountMicros
                        : "",
    ...
    store.when("product", "finished", function(product) {
        ...
        if (
            product.type === store.CONSUMABLE ||
            product.type === store.NON_RENEWING_SUBSCRIPTION
        ) {
    ...
    function retry(fn) {
        ...
        if (retryTimeout > 120000) retryTimeout = 120000;
    }
    ...
    store.extendAdditionalData = function(product) {
        ...
        if (product.group && !a.oldSku) {
            store.getGroup(product.group).forEach(function(otherProduct) {
                if (isPurchased(otherProduct)) a.oldSku = otherProduct.id;
            });
        }
    };
...
(function() {
    ...
    store.setProductData = function(product, data) {
        ...
        if (
            typeof data.getPurchaseState !== undefined &&
            data.getPurchaseState > 0
        ) {
    ...
    store.iabUpdatePurchases = function(purchases) {
        ...
        if (purchases && purchases.length) {
            for (var i = 0; i < purchases.length; ++i) {
                ...
                if (!p) {
                    store.log.warn(
                        "plugin -> user owns a non-registered product: " +
                            purchase.productId
                    );
                    continue;
                }
